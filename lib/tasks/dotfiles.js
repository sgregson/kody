    ...
    if (dotFiles.length > 0) {
      ...
      try {
        for (var _iterator = dotFiles[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          var _basename = _dotFile;
          if (_basename.indexOf('/') !== -1) _basename = _basename.substr(_basename.lastIndexOf('/') + 1);
          _basename = _basename.replace(PROPS.FILE_SUFFIX, '');
          var destination = $HOME + '/.' + _basename;
          ...
          if (!shell.error()) {
            ...
            winston.error('unable to link ' + _basename + ' to ' + destination);
          }
        }
      } catch (err) {
